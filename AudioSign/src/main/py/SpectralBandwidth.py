...
def compareAudioSamples():
    ...
    # Extract spectral bandwidth from the speech segments
    refSpectralBandwidth = librosa.feature.spectral_bandwidth(y=refAudio[refSpeechSegment[0]:refSpeechSegment[1]], sr=refSampleRate)
    testSpectralBandwidth = librosa.feature.spectral_bandwidth(y=testAudio[testSpeechSegment[0]:testSpeechSegment[1]], sr=testSampleRate)

    # Combine MFCCs and spectral bandwidth features
    refFeatures = np.vstack([refMFCCs, refSpectralBandwidth])
    testFeatures = np.vstack([testMFCCs, testSpectralBandwidth])
